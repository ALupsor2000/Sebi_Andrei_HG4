...
public class RegistrationSystem {
    ...
    public void insert_stud() throws DataExceptions {
        ...
        try {
            // preluam informatiile despre studentul pe care vrem sa il inseram
            ...
            //inseram in fisier si golim lista students
            Student s = new Student(id, firstname, lastname, 0, new ArrayList<>());
            //preiau din fisier toti studentii intr-o lista de tip Student
            List<Student> student = studentFile.read_to_list();
            // adaug in lista studentul dorit
            student.add(s);
            // rescriu fisierul
            studentFile.edit(student);
            student.clear();
    ...
    public void delete_stud() throws DataExceptions {
        ...
        try {
            // id-ul studentului pe care dorim sa il stergem
            ...
            for (Student student : students) {
                ...
                if (student.getStudentId() == (id_stud)) {
                    ...
                    // scoatem din lista students studentul, apoi folosind edit rescriem fisierul fara acet student
                    students.remove(student);
                    studentFile.edit(students);
                    // golim lista students pentru urmatoarea executie
                    students.clear();
                } else {
    ...
    public void insert_teacher() throws DataExceptions {
        ...
        try{
            ...
            //preiau din fisier toti profesorii intr-o lista de tip Teacher
            List<Teacher> profesor = teacherFile.read_to_list();
            // adaug in lista profesorul dorit
            profesor.add(t);
            // rescriu fisierul
            teacherFile.edit(profesor);
            profesor.clear();
        } catch (InputMismatchException e) {
    ...
    public void insert_course() throws DataExceptions {
        try {
            // id-ul profesorului caruia dorim sa ii adaugam un curs nou
            System.out.println("Insert teacher id: ");
            long id_teacher = scan.nextLong();

            //preluam toti profesorii din fisier
            ...
            for (Teacher teacher : teachers) {
                ...
                if (teacher.getId() == id_teacher) {
                    ...
                    // cursul nou
                    Course c = new Course(id_course, name, teacher, enrolled_places, new ArrayList<>(), credits);

                    //preiau din fisier toate cursurile intr-o lista de tip Course
                    List<Course> cursuri = courseFile.read_to_list();
                    // adaug in lista cursul dorit
                    cursuri.add(c);
                    // rescriu fisierul
                    courseFile.edit(cursuri);
                    cursuri.clear();

                    //stochez lista de cursuri a profesorului
                    List<Course> teacher_course_list = teacher.getCourses();
                    //adaug in acea lista noul curs
                    teacher_course_list.add(c);
                    teacherFile.edit(teachers);
                } else {
                    System.out.println("Teacher id does not exist");
                }
            }
            //golim lista pentru urmatoarea executie
            teachers.clear();
        }catch (InputMismatchException e) {
    ...
    public void delete_course(){
        ...
        for(Teacher teacher : teachers) {
            if (teacher.getId().equals(id_teacher)) {
                ...
                // preia cursurile profesorului in lista
    ...
    /**
     *
     * @param id_student id of student to register
     * @return true if student was succsesfully added to course, false otherwise
     */
        ...
        try {
            ...
            for (Course course : courses) {
                if (course.getId().equals(id_cours)) {
                    ...
                    //verificam daca studentul va avea in urma adaugarii un numar mai mare de 30 de credite
                    else {
                        for (Student student : students) {
                            if (student.getStudentId() == (id_student)) {
    ...
    /**
     *
     * @return list of courses, wich has empty places left
     */
    ...
    /**
     *
     * @param id_course id for course to show student list
    ...
    /**
     *
     * @param courses list of courses we want to filter
     * @return filtered list of courses
     */
    public static int Filtercredits(List<Course> courses){
        //filtreaza cursurile care au numaru de credite mai mare decat 5
